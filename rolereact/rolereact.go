...
func MessageModifyData(i *gokord.InteractionCreate, parameters *EditID) *interaction.ResponseData {
...
func MessageModifyRoleComponents(i *gokord.InteractionCreate, parameters *EditIDWithRole, emojiMessage string) []component.Message {
	message, ok := GetMessageFromEditID(i, parameters.MessageEditID)
...
func MessageModifyRoleData(i *gokord.InteractionCreate, parameters *EditIDWithRole, emojiMessage string) interaction.ResponseData {
	components := []component.Component{}
...
func HandleCommand(
	...
	if err != nil {
		...
		if err != nil {
			s.LogError(err, "Unable to send message")
		}
		return
	}
	...
	err = cfg.Save()
	if err != nil {
		s.LogError(err, "Unable to save rolereact message in database")
		err := s.InteractionAPI().Respond(i.Interaction, &interaction.Response{
	...
	if err != nil {
		s.LogError(err, "Unable to send edit rolereact message")
	}
}
...
func HandleModifyCommand(
	...
	if targetEditID == 0 {
		...
		if target == nil {
			...
			if err != nil {
				s.LogError(err, "Unable to send rolereact message not found")
			}
			return
		}
	...
	if err != nil {
		s.LogError(err, "Unable to send modify rolereact message")
	}
}
...
func HandleResetMessage(
	...
	if err != nil {
		s.LogError(err, "Unable to send reset message message")
	}
}
...
func HandleStartSetNote(
	...
	if err != nil {
		s.LogError(err, "Unable to send edit note modal")
	}
}
...
func HandleSetNote(
	...
	if err != nil {
		s.LogError(err, "Unable to send updated note message")
	}
}
...
func HandleApplyMessage(
	...
	if !ok {
		...
		if err != nil {
			s.LogError(err, "Unable to defer interaction")
			return
		}
		messageContent := MessageContent(message)
		...
		if err != nil {
			...
			s.LogError(err, "Unable to apply rolereaction message changes")
			err = nil
		} else {
		...
		if err != nil {
			s.LogError(err, "Unable to send apply rolereaction message changes")
		}
	}
}
...
func HandleNewRole(
	...
	if err != nil {
		s.LogError(err, "Unable to send modify reaction role message")
	}
}
...
func HandleOpenRole(
	...
	if err != nil {
		s.LogError(err, "Unable to send open reaction role message")
	}
}
...
func HandleSetReaction(
	...
	cancelHandler := s.AddHandler(func(s *gokord.Session, e *gokord.MessageReactionAdd) {
	...
	case emoji := <-emojiChann:
		...
		if err != nil {
			...
			if err != nil {
				s.LogError(err, "Unable to edit original response")
			}
			return
		}
...
func HandleDelRole(
	...
	if err != nil {
		s.LogError(err, "Unable to send modify message message")
	}
}

func GetMessageFromEditID(i *gokord.InteractionCreate, editID uint) (*config.RoleReactMessage, bool) {
